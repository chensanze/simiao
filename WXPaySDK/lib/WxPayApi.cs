    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", WxPayConfig.APPID);//公众账号ID
            inputObj.SetValue("mch_id", WxPayConfig.MCHID);//商户号
            inputObj.SetValue("device_info", WxPayConfig.DEVICEINFO);//设备号 公众号需传入WEB
            //终端ip  APP和网页支付提交用户端ip，Native支付填调用微信支付API的机器IP。 csz 此处使用用户IP 在外部赋值 
        ...
        */
        ...
        {
            var ran = new Random();
            return string.Format("{0}{1}{2}", WxPayConfig.MCHID, DateTime.Now.ToString("yyyyMMddHHmmssfff"), ran.Next(99999));
        }
